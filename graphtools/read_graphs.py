...
if os.path.exists(f'{mews}/outputs/nodes/{filename}.out') \
    ...
    with open(f'{mews}/outputs/nodes/{filename}.out', 'r') as nodes_file, \
            ...
            open(f'{mews}/outputs/classification_results/{filename}', 'w+') as results_file:
        ...
        print('Number of features selected by the solver', len(all_feature_indices), file=results_file)
        ...
        for choice in ['throw median']:
            if all_feature_indices != []:
                ...
                for classifier in ['SVC', 'RF', 'MLP']:
                    print('Choice and classifier', choice, classifier, file=results_file)
