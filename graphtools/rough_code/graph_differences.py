...
def different_graphs(fscores, mat, big5, data, whole, labels, corr, mews):
    ...
    with open('outputs/combined_params.json', 'r') as f:
        ...
        for i in range(5):  # different labels
            ...
            params = best_params['RF'][big5[i]]["100"][choice]  # maybe use the parameters that work the best for top 5%
            ...
            for edge, arr in zip(['fscores', 'pearson', 'feature_importance'],
                ...
                thresh = np.percentile(np.absolute(arr), 10)  # remove bottom 5% in absolute terms
                ...
                for feature in ['mean_FA', 'mean_strl', 'num_str']:
                    ...
                    for j in range(len(mat[0])):
                        if feature == 'mean_FA':
                            edge_attributes.append((mat[0][j], mat[1][j], arr[0, j]))
                        if feature == 'mean_strl':
                            edge_attributes.append((mat[0][j], mat[1][j], arr[1, j]))
                        if feature == 'num_str':
                            edge_attributes.append((mat[0][j], mat[1][j], arr[2, j]))
