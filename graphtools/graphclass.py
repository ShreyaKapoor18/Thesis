...
class Graph(nx.Graph): #inheriting from networkx graph package along with the functionality we want
    ...
    def set_node_labels(self, node_labels, node_wts):
        ...
        for l in self.nodes.keys():
            if node_wts == 'max':
                self.nodes[l]['label'] = max([[l][k]['weight'] for k in self[l].keys()])  # max or max abs?
            elif node_wts == 'const':
    ...
    def visualize_graph(self, mews, connected, threshold, plotting_options):
        ...
        edge_wts = []
        for m in self.connected_subgraph.edges.data():
            # print(m)
        ...
        if connected:  # to visualize the connected subgraph in the input
            plt.title(f'Nodes with degree >={self.connected_subgraph.degree}, input to the solver: {self.filename}\n Number of edges {len(self.edges)}\n'
            ...
            nx.draw(self.connected_subgraph, **plotting_options, edge_color=color)
    ...
    def savefiles(self, mews, degree):
        ...
        self.connected_subgraph.degree = degree
    ...
    def read_from_file(self, mews, mat=np.triu(84)):
