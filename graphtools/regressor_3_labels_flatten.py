...
def plot_regression_results(ax, y_true, y_pred, title, scores, elapsed_time):
    ...
    ax.set_title(title)
#%%
def dict_regressor(whole, metrics, labels, data, new_fscores):
    """
    ...
    for i in range(5):# different labels
        ...
        for per in [5, 10, 15, 20]:
                ...
                plt.suptitle(f'Single predictors versus stacked predictors for label {big5[i]} and'
                             f'top {per}% features')
                plt.tight_layout()
                plt.subplots_adjust(top=0.9)
                plt.savefig(f'outputs/{big5[i]}_{per}')
...
metrics = ['explained_variance', 'max_error', 'r2', 'neg_mean_absolute_error']
cv_metrics = ['balanced_accuracy', 'roc_auc', 'accuracy', 'f1']
dict_regressor(whole, metrics, labels, data, new_fscores)
