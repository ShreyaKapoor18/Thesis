...
targets = ['Agreeableness', 'Openness', 'Conscientiousness', 'Neuroticism',
        'Extraversion']
edges = ['pearson', 'fscore', 't_test']
node_wtsl = ['max', 'const']
factors = [1000]
sub_vals = [1000/5, 1000/10, 1000/20, 1000/25]
...
for j, (feature_type, target, edge, node_wts, sub_val,factor) in \
        enumerate(itertools.product(feature_types,targets,edges, node_wtsl, sub_vals, factors)):
    print('*'*100)
    ...
    arr = arr.abs()
    arr = pd.DataFrame(scale(arr), index=arr.index)# scale the array according to the index
    arr = arr * factor
