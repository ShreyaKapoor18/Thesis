    ...
    with open('outputs/combined_params.json', 'r') as f:
        ...
        for i in range(5):  # different labels
            ...
            for edge in ['fscores', 'pearson', 'feature_importance']:
                ...
                filename = f'{big5[i]}{edge}'#make more nested directories
                nodes_file = open(f'{mews}/outputs/nodes/{big5[i]}{edge}', 'w+')
                edges_file = open(f'{mews}/outputs/edges/{big5[i]}{edge}', 'w+')
                count = 0
                for x in g1.nodes:
                    # print(node)
                    if g1.degree(x) >= 2:
